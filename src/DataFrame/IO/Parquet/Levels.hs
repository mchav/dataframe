...
readLevelsV1 n maxDef maxRep bs =
  let bwDef = bitWidthForMaxLevel maxDef
      ...
      (repLvlsU32, afterRep) =
        if bwRep == 0 then (replicate n 0, bs)
        else 
      ...
      (defLvlsU32, afterDef) =
        if bwDef == 0 then (replicate n 0, afterRep)
        else 
...
readLevelsV2 n maxDef maxRep defLen repLen bs =
  let (repBytes, afterRepBytes) = splitAt (fromIntegral repLen) bs
      ...
      bwRep = bitWidthForMaxLevel maxRep
      (repLvlsU32, _) =
        if bwRep == 0 then (replicate n 0, repBytes)
        else decodeRLEBitPackedHybrid bwRep n repBytes
      (defLvlsU32, _) =
        if bwDef == 0 then (replicate n 0, defBytes)
...
parseOne (se:rest) =
  ...
  in ( SNode { sName = T.unpack (elementName se)
