...
import System.IO.MMap ( mmapFileForeignPtr
                      , Mode (WriteCopy)
                      )
import qualified Data.Vector.Storable as VS
import qualified Data.Vector as Vector
import Data.Vector.Storable.Mutable ( unsafeFromForeignPtr
                                    , grow
                                    )
...
fastReadCsvUnstable = readCsvUnstable'
                        defaultReadOptions
                        getDelimiterIndices

readCsvUnstable :: FilePath -> IO DataFrame
readCsvUnstable = readCsvUnstable'
                    defaultReadOptions
                    (\v -> do
                      let len = VS.length v
...
readCsvUnstable' opts delimiterIndices filePath = do
  ...
  (bufferPtr, offset, len) <- mmapFileForeignPtr
                                filePath
                                WriteCopy
                                Nothing
  ...
  let numCol = countColumnsInFirstRow paddedCSVFile indices
      ...
      (columnNames, dataStartRow) = case headerSpec opts of
        NoHeader -> 
          (Vector.fromList
          $ map (Text.pack . show) [0 .. numCol - 1], 0)
        UseFirstRow -> 
          (Vector.fromList
          $ map extractField' [0 .. numCol - 1], 1)
        ProvideNames ns -> 
          (Vector.fromList ns, 0)
      numRow = totalRows - dataStartRow
      parseTypes col
        = let n = if shouldInferFromSample (typeSpec opts)
...
getDelimiterIndices csvFile
  ...
  -- empty fields)
  indices <- mallocArray len
  num_fields <- get_delimiter_indices
                  (castPtr buffer)
                  (fromIntegral len)
                  (castPtr indices)
  if num_fields == -1
