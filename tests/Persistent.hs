...
withTestDb :: (SqlBackend -> IO a) -> IO a
...
testFromPersistent = TestCase $ withTestDb $ \backend -> do
    -- Insert test data
    ...
    liftIO $ do
        -- Check row count
        assertEqual "Row count" 4 (DF.nRows df)
        ...
        -- Check data values
        let names = DF.columnAsVector @Text "name" df
        assertEqual "Names match" ["Alice", "Bob", "Charlie", "Diana"] 
                    (DF.toList names)

-- Test loading with filters
testFromPersistentWithFilters :: Test
testFromPersistentWithFilters = TestCase $ withTestDb $ \backend -> do
    -- Insert test data
    mapM_ insert testUsers
    
    -- Load active users only
    df <- fromPersistent [TestUserActive ==. True]
    liftIO $ do
        assertEqual "Active users count" 3 (DF.nRows df)
        
        -- Check all loaded users are active
        let activeFlags = DF.columnAsVector @Bool "active" df
        assertBool "All active" (all id $ DF.toList activeFlags)

-- Test custom configuration
testCustomConfig :: Test
testCustomConfig = TestCase $ withTestDb $ \backend -> do
    -- Insert test data
    mapM_ insert testUsers
...
testWithoutIdColumn = TestCase $ withTestDb $ \backend -> do
    -- Insert test data
    mapM_ insert testUsers
...
testDataFrameOperations = TestCase $ withTestDb $ \backend -> do
    -- Insert test data
    mapM_ insert testUsers
    ...
    liftIO $ do
        ...
        assertEqual "Young users count" 2 (DF.nRows youngUsers)
        
        -- Sort operation
        let sorted = DF.sortBy DF.Ascending ["age"] df
        let ages = DF.toList $ DF.columnAsVector @Int "age" sorted
...
testWithRelationships = TestCase $ withTestDb $ \backend -> do
    ...
    liftIO $ assertEqual "Order count" 3 (DF.nRows ordersDF)
    
    -- Load users
    usersDF <- fromPersistent @TestUser []
    
    -- TODO: Add join operations when available
    liftIO $ assertEqual "User count" 2 (DF.nRows usersDF)

-- Test empty result set
testEmptyResultSet :: Test
testEmptyResultSet = TestCase $ withTestDb $ \backend -> do
    -- Don't insert any data
    df <- fromPersistent @TestUser []
    liftIO $ do
        assertEqual "Empty DataFrame" 0 (DF.nRows df)
