...
main = do
    ...
    zipWithM_
        ...
        [0 .. 3]
        (classwisePrecision confusionTrain)
    putStrLn "======= Classwise Recall ======="
    zipWithM_
        (\a b -> putStrLn $ show a ++ "| " ++ show b)
        [0 .. 3]
        (classwiseRecall confusionTrain)
    ...
    zipWithM_
        (\a b -> putStrLn $ show a ++ "| " ++ show b)
        [0 .. 3]
        (classwisePrecision confusionTest)
    putStrLn "======= Classwise Recall ======="
    zipWithM_
        (\a b -> putStrLn $ show a ++ "| " ++ show b)
        [0 .. 3]
        (classwiseRecall confusionTest)
...
-- This stores our counts.
type ConfusionMatrix = A.Array (Int, Int) Float

-- This function computes a confusion matrix.
-- Note: actual values are along the vertical axis of the confusion matrix,
-- while predicted values are along the horizontal axis.
-- If you use this function, it is important tp pass in actual and predicted
-- in the order that is specified below, otherwise the columns and rows will
-- be switched
