...
* **Column**: a wrapper around the vector that stores the column data.
* **TypedColumn a**: statically typed column with phantom type `a` (e.g., `Double`, `Text`, `Bool`).
...
```haskell
-- Creates expressions representing the each of the dataframe columns.
$(declareColumnsFromCsvFile "./data/iris.csv")
...
-- OR let df2 = D.select [F.name sepal_length, F.name sepal_width, F.name variety]

-- Note: Renaming will break the column bindings generated by template haskell.
